...
def init_models(type='classification'):
    if type == 'regression':
        ...
        models = {
            ...
            # 'dt_classifier_pipeline': Pipeline([('classifier', DecisionTreeClassifier(random_state=42))]),
            ...
            'xgb_classifier_pipeline': Pipeline([('classifier', XGBClassifier(random_state=42))])
        }
...
def eval_per_order_status(x, y, models, type):
    # order_status_list = ['PENDING', 'PROCESSING', 'PENDING_PAYMENT', 'PAYMENT_REVIEW', 'ON_HOLD', 'PAYMENT_REVIEW', 'CLOSED']
...
def init_nn(params, type='regression'):
    if type == 'regression':
        models = {
